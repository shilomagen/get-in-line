...
import { UserDomain } from './src/services/user';
...
export const notifyAppointmentSet = async (event: any, context: any) => {
  ...
  const content = message.status === ResponseStatus.Success ? Messages.scheduleSuccess(message.data) : errors[message.data.errorCode](message.user!);
  if (content) {
